// 経県値マップ 2026 — 静的デモ版(GitHub Pages 用)
// サーバ無し。塗りは localStorage(この端末のブラウザ)にのみ保存され、揮発しうる。
// ログイン機能は無し。記録の保管は CSV/HTML/JPG ダウンロードで行う。

const STORE_KEY = "keikenshi2026"; // localStorage キー

// ---- ジオメトリ source -------------------------------------------------
const GEOJSON_URL = "cityarea.geojson"; // 市区町村境界(1894)

// ---- 経県値の段階(本家準拠) --------------------------------------------
const LV_MAX = 4;
const LV_NAME = { 1: "通過", 2: "接地", 3: "訪問", 4: "宿泊" };
const LV_HEX = { 1: "#00ffff", 2: "#00cc00", 3: "#ffff00", 4: "#ff0000" };
...
const fillColor = [
  ...
  1, LV_HEX[1], 2, LV_HEX[2], 3, LV_HEX[3], 4, LV_HEX[4],
  /* 0/未踏 */ "#ffffff",
];
...
// 各種データのクレジットは右下に(表示中のソースの分だけ自動表示)
...
map.on("load", async () => {
  ...
  map.addSource(SOURCE, { type: "geojson", data: GEOJSON_URL, promoteId: "code", attribution: N03_ATTR });
  ...
  map.addLayer({
    ...
    layout: { visibility: "none", "line-cap": "round" },
    paint: { "line-color": "#b5402a", "line-width": ["interpolate", ["linear"], ["zoom"], 6, 1.0, 12, 3.0] },
  });
  map.addLayer({
  ...
  // 自治体名ラベル(初期OFF)
  map.addLayer({
    id: "labels", type: "symbol", source: SOURCE,
    layout: {
...
function setLv(code, lv) {
  paints[code] = lv;
  map.setFeatureState({ source: SOURCE, id: code }, { lv });
}
...
function saveData() {
  ...
  try {
    localStorage.setItem(STORE_KEY, JSON.stringify(paints));
    status.textContent = "保存(この端末) ✓";
  } catch (_) {
